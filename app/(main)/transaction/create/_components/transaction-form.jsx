...
function AddTransactionForm({ accounts, categories, editMode, initialData }) {
  ...
  const handleScanSubmit = (scannedData) => {
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      {!editMode && <ReceiptScanner onScanComplete={handleScanSubmit} />}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              {accounts.map((account) => (
                <SelectItem
                  // className={`${account.balance < 0 ? "text-rose-500" : "text-teal-500"}`}
                  key={account.id}
                  value={account.id}
                >
          ...
          {errors.accountId && (
            <p className="text-sm text-rose-500">{errors.accountId.message}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className="w-full text-left font-medium">
              {date ? format(date, "PPP") : <span>Pick a Date</span>}
              <Calendar1Icon className="ml-auto text-violet-600 h-4 w-4 opacity-50" />
            </Button>
          </PopoverTrigger>
        ...
        {errors.date && (
          <p className="text-sm text-rose-500">{errors.date.message}</p>
        )}
      </div>
