...
export const ProcessedRecurringTransactions = inngest.createFunction(
  {
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`generate-report-${user.id}`, async () => {
        ...
        const stats = await getMontlyStats(user.id, lastMonth);
...
const getMontlyStats = async (userId, month)=>{
  ...
  return transactions.reduce(
    (stats, t) => {
      ...
      if(t.type === "EXPENSE"){
        ...
        stats.byCategory[t.category] = (stats.byCategory[t.category] ||0) + " " + amount.toFixed(2)
      }else{
        stats.totalIncome +=amount.toFixed(2);
      }
